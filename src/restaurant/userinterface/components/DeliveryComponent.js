...
export default function RestaurantComponent({ data }) {
  ...
  return (
    ...
    Restaurants in {data[0]?.cityname || "your city"}
    </div>
    ...
    {data.map((item, index) => (
    <Paper onClick={()=>navigate(`/restaurantfooddetails/${item?.restaurantid}/${item?.restaurantname}`)}  key={index} elevation={3} style={{borderRadius: "18px",cursor:'pointer', overflow: "hidden",width: "min(100%, 350px)",display: "flex",flexDirection: "column",transition:"transform 0.2s ease, box-shadow 0.2s ease",boxShadow:"0 8px 24px rgba(15,23,42,0.10)"}}>
